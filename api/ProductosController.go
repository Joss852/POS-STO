...
(codigoBarras, descripcion, embalaje, precioCompra, precioVenta, existencia, stock)
VALUES
(?, ?, ?, ?, ?, ?, ?)`)
		if err != nil {
		...
		for indiceFila := comenzarDesde; indiceFila < cantidadDeFilas; indiceFila++ {
			...
			if ajustesDeImportacion.IndiceEmbalaje > 0 && ajustesDeImportacion.IndiceEmbalaje <= cantidadCeldas {
				embalaje, _ = celdas[ajustesDeImportacion.IndiceEmbalaje-1].String()
			} else {
				log.Printf("Error. El índice del embalaje debe ser mayor o igual que 0. En cambio, se intentó dar %d", ajustesDeImportacion.IndiceEmbalaje)
				return false
			}
...
func (p *ProductosController) exportarCSV(ajustes *AjustesParaExportarProductos) {
	...
	filas, err := db.Query(`SELECT idProducto, codigoBarras, descripcion, embalaje, precioCompra, precioVenta, existencia, 
stock FROM productos ORDER BY idProducto DESC;`)
	...
	if ajustes.IncluirEncabezado {
		archivo.WriteString("idProducto,codigoBarras,descripcion,embalaje,precioCompra,precioVenta,existencia,stock\n")
	}
	var producto Producto
	for filas.Next() {

		err := filas.Scan(&producto.Numero, &producto.CodigoBarras, &producto.Descripcion, &producto.Embalaje,
		...
		for contador := 0; contador < ajustes.Copias; contador++ {
			archivo.WriteString(fmt.Sprintf("%d,%s,%s,%s,%0.2f,%0.2f,%0.2f,%0.2f\n",
				producto.Numero, producto.CodigoBarras, producto.Descripcion, producto.Embalaje,
				producto.PrecioCompra, producto.PrecioVenta, producto.Existencia, producto.Stock))
		}
		if err != nil {
...
stock FROM productos ORDER BY idProducto DESC;`)
	...
	if ajustes.IncluirEncabezado {
		fila = hoja.AddRow()
		for _, encabezado := range []string{"idProducto", "codigoBarras", "descripcion", "embalaje", "precioCompra", "precioVenta", "existencia", "stock"} {
			celda = fila.AddCell()
	...
	for filas.Next() {
		...
		for contador := 0; contador < ajustes.Copias; contador++ {
			...
			celda = fila.AddCell()
			celda.SetString(producto.Embalaje)
			celda = fila.AddCell()
